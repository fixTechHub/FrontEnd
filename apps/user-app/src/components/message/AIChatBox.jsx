...
const AIChatbox = () => {
    ...
    return (
        <div className="position-fixed bottom-0 end-0 mb-3 me-3 z-3 font-sans">
            ...
            {/* Chatbox */}
            <div
                className={`bg-white rounded-3 shadow-lg d-flex flex-column transition-all duration-500 ease-in-out position-absolute bottom-100 end-0 border border-light-subtle ${isOpen ? '' : 'translate-y-100 opacity-0 pointer-events-none'
                    }`}
            ...
            >
                ...
                >
                    ...
                    {messages.map((msg, index) => (
                        <div
                            key={index}
                            className={`p-2 mb-2 rounded-2 small text-start ${msg.sender === 'user'
                                ? 'ms-auto bg-primary-subtle text-primary-emphasis'
                                : 'bg-white text-dark border border-light-subtle'
                                }`}
                            style={{ maxWidth: '80%', wordWrap: 'break-word' }}
                        >
                            <ReactMarkdown>{msg.text}</ReactMarkdown> {/* Use ReactMarkdown to render message text */}
                    ...
                    {status === 'loading' && (
                        <div className="p-2 mb-2 bg-white rounded-2 small d-flex align-items-center border border-light-subtle">
                            <div className="d-flex gap-1 me-2">
                                ...
                                <span className="d-block w-2 h-2 bg-primary rounded-circle animate-bounce" style={{ animationDelay: '0.1s' }}></span>
                                <span className="d-block w-2 h-2 bg-primary rounded-circle animate-bounce" style={{ animationDelay: '0.2s' }}></span>
                            </div>
                            <span className="text-muted fst-italic">Đang xử lý...</span>
                        </div>
                    )}
                    {error && (
                        <div className="text-danger text-center small py-2">{error}</div>
                    )}
                </div>
